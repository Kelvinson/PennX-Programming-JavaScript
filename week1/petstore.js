...
function calculateFoodOrder(numAnimals, avgFood) {
     if ( isNaN(parseFloat(numAnimals)) || !isFinite(numAnimals) || isNaN(parseFloat(avgFood)) || !isFinite(avgFood) || numAnimals < 0 || avgFood < 0 ) {
        return -1;
     } else {
 ...
 * @return a string containing the name of the most popular day of the week if there is only one most popular day, 
    and an array of the strings containing the names of the most popular days if there are more than one that are 
    most popular.
 */
function mostPopularDays(week) {
    ...
    for (var i = 0; i < week.length; i++) {
        ...
        }else if (week[i].traffic == largest)
        {
...
function createAnimalObjects(names, types, breeds) {
     ...
     for (var i = 0; i < x; i++) {
        animalCreated.push(new Animal(names[i], types[i],breeds[i]));
     }
     return animalCreated;
}
